...
class LikeAnswerView(LoginRequiredMixin, generic.edit.CreateView):
	...
	def form_valid(self, form):
		...
		if not UserLikes.objects.filter(user=self.request.user, answer=answer) and not UserDislikes.objects.filter(user=self.request.user, answer=answer):
			...
			answer.save()
			like.save()
			return super(LikeAnswerView, self).form_valid(form)
		else:
			answer = get_object_or_404(Answer, id=self.request.POST.get('answer'))
			return redirect('faq:detail', answer.question.pk)
...
class DislikeAnswerView(LoginRequiredMixin, generic.edit.CreateView):
	...
	def form_valid(self, form):
		...
		if not UserLikes.objects.filter(user=self.request.user, answer=answer) and not UserDislikes.objects.filter(user=self.request.user, answer=answer):
			...
			answer.save()
			dislike.save()
			return super(DislikeAnswerView, self).form_valid(form)
		else:
			answer = get_object_or_404(Answer, id=self.request.POST.get('answer'))
			return redirect('faq:detail', answer.question.pk)
...
class BestAnswerView(LoginRequiredMixin, generic.edit.UpdateView):
	...
	def form_valid(self, form):
		...
		if question.user == self.request.user:
			...
			answer.save()
			return super(BestAnswerView, self).form_valid(form)
		else:
			return redirect('faq:detail', answer.question.pk)